...
import InviteAcceptedModal from "@/src/widgets/agency-invites/ui/InviteErrorModal";
import { useAuth } from "@/src/shared/providers/authProvider";
import { useCreateAgencyMemberMutation } from "@/src/entities/members/api/membersApi";
...
const AgencyInviteCard: React.FC<AgencyInviteCardType> = ({ data, token }) => {
  ...
  return (
    <div className="mx-auto w-full max-w-md">
      {modalShow || (!invite && <InviteAcceptedModal />)}
      <Card className="border-border/70 shadow-sm">
        <CardHeader className="space-y-4 pb-3">
          ...
          <div className="flex items-center px-1 gap-2">
            ...
            <Badge variant={"outline"}>{roles[invite?.role || "member"]}</Badge>
          </div>
        </CardHeader>
        ...
        <CardContent className="space-y-4">
          <p className="text-sm leading-relaxed text-foreground/90">
            ...
            <span className="font-semibold">{invite?.agency_name}</span>. После
            принятия вы получите доступ к проектам и участникам команды.
          </p>
        ...
        <CardFooter className="grid grid-cols-2 gap-2">
          ...
          >
            ...
            {isAcceptLoading ? "Принятие..." : accepted ? "Принято" : "Принять"}
          </Button>
        </CardFooter>
        {acceptError && (
...
export default AgencyInviteCard;
